...
def test_handle_new_user_is_security_definer() -> None:
    migrations = "\n".join(path.read_text(encoding="utf-8") for path in (Path(__file__).resolve().parents[3] / "supabase/migrations").glob("0*.sql"))
...
def test_elevated_connection_module_documents_allow_list() -> None:
    text = (APP_ROOT / "repositories/system_tasks.py").read_text(encoding="utf-8")
