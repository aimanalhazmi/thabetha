...
1. **Pin the constitution.** Symlink (or copy) `claude-handoff/constitution.md` into `.specify/memory/constitution.md` once. Re-run `/constitution` only if rules change.
2. **`/specify "<problem statement>"`** — paste the *Problem Statement* from the phase. spec-kit will produce `spec.md` in a new `specs/NNN-<slug>/` folder.
3. **`/clarify`** — let spec-kit ask its clarification questions. Use the *Pre-answered clarifications* in each phase to answer quickly and consistently.
4. **`/plan`** — feed in the *Technical context* from the phase (stack, constraints, files to touch). spec-kit produces `plan.md` + design artifacts.
5. **`/tasks`** — produces an ordered, dependency-aware task list.
6. **`/analyze`** (optional) — cross-check spec/plan/tasks consistency before coding.
7. **`/implement`** or hand tasks to a teammate. Each phase lists *Acceptance signals* — these are what closes the phase.
8. **After merge:** update `claude-handoff/project-status.md` and the relevant row in `claude-handoff/use-cases.md`. Tick the phase below.
...
# Team workflow (when working in parallel)

- One developer per phase. Sub-tasks within a phase (from `/tasks`) can be parallelized within that branch.
- Phases in the same block can run in parallel **if** their dependency table above is clear; e.g. Phases 1, 2, 3 are independent and can be three parallel branches off `develop`.
- Always rebase on `develop` before opening a PR. spec-kit's `specs/NNN-*/` folders are designed to be merge-conflict-free, but the shared docs (`project-status.md`, `use-cases.md`) will conflict — resolve with the latest-merge-wins convention and re-run the affected status updates.
- Daily standup question: *which phase, which `/specify | /plan | /tasks | /implement` step, what's blocking?* — keeps the spec-kit state visible.
