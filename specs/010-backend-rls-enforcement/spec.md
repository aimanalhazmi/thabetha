...
- Connection pooling is provided by PgBouncer (or equivalent). The chosen identity-scoping mechanism must work under the pooler configuration in use; if a particular path requires session-level state, it opts into a session-pooled connection instead of transaction-pooled.
