...
- [X] T001 Create the WhatsApp service module skeleton: `backend/app/services/whatsapp/__init__.py` (empty), and ensure `backend/tests/services/whatsapp/__init__.py` and `backend/tests/api/` exist (create the directories if missing).
...
- [X] T002 Add migration `supabase/migrations/009_whatsapp_delivery.sql` adding `whatsapp_attempted boolean NOT NULL DEFAULT false`, `whatsapp_delivered boolean`, `whatsapp_provider_ref text`, `whatsapp_failed_reason text`, `whatsapp_status_received_at timestamptz` to `notifications`, plus the unique partial index `notifications_whatsapp_provider_ref_key` on `whatsapp_provider_ref WHERE NOT NULL` and the `notifications_whatsapp_provider_ref_idx` lookup index per `data-model.md`.
- [X] T003 Extend `Settings` in `backend/app/core/config.py` with `whatsapp_phone_number_id: str | None = None`, `whatsapp_webhook_secret: str | None = None`, `whatsapp_verify_token: str | None = None` (the existing `whatsapp_provider`, `whatsapp_access_token`, `whatsapp_from_number` stay).
- [X] T004 [P] **Verify & extend** the WhatsApp schema shapes already present in `backend/app/schemas/domain.py` (as of the feature branch): `WhatsAppDeliveryStatus`, `derive_whatsapp_status`, `NotificationOut`, and `NotificationOutCreditor` are already defined. Confirm they match the contract in `data-model.md` exactly. Add any missing fields — specifically ensure `NotificationOutCreditor` includes `whatsapp_status_received_at: datetime | None = None` (it was absent in the initial implementation) and that `frontend/src/lib/types.ts` mirrors the full shape (T005).
- [X] T005 [P] Mirror the new enum and the `NotificationOutCreditor` shape in `frontend/src/lib/types.ts`, including the `whatsapp_status_received_at: string | null` field (ISO datetime string on the TS side). Keep the debtor-facing `NotificationOut` unchanged. `WebhookReceiptOut` is backend-internal and does not need a frontend mirror.
- [X] T006 Define the `WhatsAppProvider` ABC plus the dataclasses `SendOutcome`, `SendRequest`, `SendResult`, `StatusUpdate` in `backend/app/services/whatsapp/provider.py`, exactly matching the contract in `contracts/whatsapp_provider.md`.
- [X] T007 **Verify & extend** the `Repository` ABC in `backend/app/repositories/base.py`. The abstract methods `mark_whatsapp_attempted`, `apply_whatsapp_status`, and `get_whatsapp_state` already exist. Confirm their signatures match `contracts/whatsapp_provider.md`. Ensure `apply_whatsapp_status` is understood to also write `whatsapp_status_received_at` from `update.occurred_at`; add a docstring if the column was not mentioned previously.
- [X] T008 [P] Implement the two new repository methods on `InMemoryRepository` in `backend/app/repositories/memory.py` with forward-only state semantics (delivered=true is sticky; failed after delivered is no-op).
- [X] T009 [P] Implement the two new repository methods on `PostgresRepository` in `backend/app/repositories/postgres.py` using the single-statement UPDATE shown in `contracts/webhook_meta.md` §Handler algorithm.
- [X] T010 [P] Update `backend/tests/conftest.py` to force `WHATSAPP_PROVIDER=mock` (in addition to the existing `REPOSITORY_TYPE=memory`). Wire a `mock_whatsapp` autouse fixture that resets `MockWhatsAppProvider.calls` between tests.
...
- [X] T020 [P] [US2] Preference tests in `backend/tests/services/whatsapp/test_dispatch_preferences.py` covering all four scenarios from spec.md US2:
...
- [X] T021 [US2] Implement the preference gate at the top of `dispatch_notification` in `backend/app/services/whatsapp/dispatch.py`: read `recipient_profile.whatsapp_enabled`; if false → leave `whatsapp_attempted=false` and return. Otherwise, when the notification is sent from a creditor to a debtor, look up `merchant_notification_preferences` row for `(creditor_id, debtor_id)`; if found and `whatsapp_enabled=false` → leave `whatsapp_attempted=false` and return. Add structured logging line `[whatsapp.dispatch] suppressed reason=<global_off|per_creditor_opt_out> user=<id> creditor=<id>`. **Edge case — deleted creditor**: if the `creditor_id` from the notification no longer resolves to an active profile (e.g., account deleted after the notification was queued), fall back to looking up the last-known preference record by `creditor_id`; if no record exists, default to the recipient's global preference. This ensures historical preference rows still enforce opt-outs for any in-flight or replayed notifications from deleted accounts.
- [X] T022 [US2] **Verify** `get_merchant_notification_preference(creditor_id, debtor_id) -> NotificationPreferenceOut | None` at `backend/app/repositories/base.py:157`. This method already exists. Confirm its signature and return type satisfy T021's call site (returning `None` for a missing preference so the dispatcher correctly falls back to the global toggle). If the existing `memory.py` and `postgres.py` implementations handle the `None` case correctly, mark this task complete with a comment; extend only if a gap is found.
...
- [X] T023 [P] [US3] Webhook handler tests in `backend/tests/api/test_webhooks_whatsapp.py` covering exactly the cases listed in `contracts/webhook_meta.md` §Tests required: `test_signature_valid_applies_status`, `test_signature_invalid_returns_403`, `test_signature_missing_returns_403`, `test_unknown_provider_ref_is_noop`, `test_duplicate_callback_idempotent`, `test_failed_after_delivered_is_noop`, `test_get_verification_handshake_ok_and_403`. Use `MockWhatsAppProvider`'s `verify_webhook_signature` plus a fixed dev secret to drive the signature path.
- [X] T024 [P] [US3] Notification-response shape tests in `backend/tests/api/test_notifications_whatsapp_visibility.py`: as the creditor (sender), `GET /api/v1/notifications` returns rows including `whatsapp_attempted`, `whatsapp_delivered`, `whatsapp_failed_reason`, `whatsapp_status`. As the debtor (recipient), the same notification's `GET` response excludes those fields entirely. Verifies Q1 / FR-006.

### Implementation for User Story 3

- [X] T025 [P] [US3] Add a `verify_whatsapp_signature(raw_body: bytes, signature_header: str, secret: str) -> bool` helper in `backend/app/services/whatsapp/dispatch.py` (NOT in `core/security.py` — placing it there would invert the layer dependency: `core/` must not import from `services/`). The function performs `hmac.compare_digest(hmac.new(secret.encode(), raw_body, sha256).hexdigest(), extracted_hex)` where `extracted_hex` is parsed from the `sha256=<hex>` format of `signature_header`. Returns `False` on any error, missing header, or wrong format. Import it directly into the webhook router in T026.
- [X] T026 [US3] Implement the webhook router in `backend/app/api/webhooks_whatsapp.py`:
    - `POST /api/v1/webhooks/whatsapp`: read `await request.body()`, call `verify_whatsapp_signature` (from T025 in `services/whatsapp/dispatch.py`). On signature failure: log WARN (never log the body), **write a rejection audit row to `debt_events`** with `event_type="whatsapp_webhook_rejected"` and `metadata={"reason": "invalid_signature", "ts": <utcnow>}` (spec edge case: "audit log entry is recorded"), return `403`. On parse error return `400`. On success: call `provider.parse_status_callback`, apply each `StatusUpdate` via `repo.apply_whatsapp_status`, return a Pydantic-modelled response `WebhookReceiptOut(received=True, applied=n)` (define `WebhookReceiptOut(received: bool, applied: int)` in `backend/app/schemas/domain.py` — do NOT return a raw dict). Log INFO per applied callback, ERROR on DB failure.
...
- [X] T027 [US3] Mount the webhook router in `backend/app/api/router.py` under `/api/v1`.
- [X] T028 [US3] Update the notifications list/detail handlers in `backend/app/api/notifications.py` to project the correct response shape. **Projection rule**: use `NotificationOutCreditor` iff `current_user.id == notification.actor_id AND notification.actor_id` is the creditor of the associated debt (i.e., the notification was sent *by* the creditor, not by the debtor). For all other callers — including a debtor viewing the same notification row as recipient — project `NotificationOut` (no delivery columns). In practice this means: when a notification is of a type where the creditor is the sender (e.g., `debt_created`, `payment_confirmed`), and the caller IS the creditor, use `NotificationOutCreditor`; otherwise use `NotificationOut`. Add a derived `whatsapp_status` computed via `derive_whatsapp_status(whatsapp_attempted, whatsapp_delivered, whatsapp_failed_reason)` per `data-model.md`. Also populate `whatsapp_status_received_at` from the DB row. Note (per constitution §IV): this handler-level filter is the only runtime guard for Q1 until a future RLS phase lands; document this in the PR description.
- [X] T029 [P] [US3] Create `frontend/src/components/WhatsAppDeliveryBadge.tsx` — a small presentational component that takes `{status: WhatsAppDeliveryStatus, failedReason?: string}` and renders a translated badge. Renders nothing for `not_attempted`. Uses i18n keys from T031.
- [X] T030 [P] [US3] Render the badge from T029 in `frontend/src/pages/NotificationsPage.tsx` only when the notification carries delivery fields (i.e. when the response is `NotificationOutCreditor`). Do not show it for `NotificationOut`. Match the existing layout — the badge sits next to the notification timestamp.
- [X] T031 [P] [US3] Add i18n strings (AR + EN) to `frontend/src/lib/i18n.ts`: `whatsapp_status_attempted`, `whatsapp_status_delivered`, `whatsapp_status_failed`, plus a `whatsapp_failed_reason_*` mapping for every code in the `failed_reason` codebook (`recipient_blocked`, `invalid_phone`, `template_not_approved`, `template_param_mismatch`, `provider_4xx`, `provider_5xx`, `network_error`, `no_template`, `no_phone_number`).
...
- [X] T032 [P] Update `docs/spec-kit/api-endpoints.md` with the `POST /api/v1/webhooks/whatsapp` and `GET /api/v1/webhooks/whatsapp` rows (note: signature-auth, not Supabase JWT).
- [X] T033 [P] Update `docs/spec-kit/database-schema.md` with the `notifications` delta from migration 009.
- [X] T034 [P] Update `docs/spec-kit/use-cases.md`: before editing, confirm the exact row label by opening the file and finding the row whose "Feature" column corresponds to "Real WhatsApp send" or "WhatsApp Business Integration". Update that row's status to ✅ once US1+US2 ship and to fully-shipped once US3 lands. (Do not assume it is labelled "UC6" without verifying.)
- [X] T035 Update `docs/spec-kit/project-status.md` — move "Real WhatsApp" from "Out of MVP scope" to "Shipped" once US3 closes.
- [X] T036 Run `quickstart.md` §Local development end-to-end with the mock provider and capture any frictions back into either `quickstart.md` or `tasks.md` follow-ups.
- [X] T037 Re-run `uv run pytest -k whatsapp` and `uv run ruff check --fix backend/app/services/whatsapp backend/app/api/webhooks_whatsapp.py` and confirm green.
- [X] T038 [P] **FR-015 regression test**: add `backend/tests/api/test_notifications_settings_ui_regression.py` (or extend an existing settings test file) verifying that the per-creditor WhatsApp preference toggle endpoint continues to work end-to-end against the mock provider. Test: (a) debtor POSTs to the notification-preference endpoint to disable WhatsApp for creditor A, (b) creditor A fires a notification, (c) assert `whatsapp_attempted=false` on the resulting notification row and `mock.calls` is empty. Then toggle back ON and repeat — assert the next notification IS attempted. This covers FR-015 and SC-007.
...
- Phase 6: T032 + T033 + T034 + T038 parallel.
...
- T025 signature helper lives in `services/whatsapp/dispatch.py`, NOT in `core/security.py`. The `core/` layer MUST NOT import from `services/`.
