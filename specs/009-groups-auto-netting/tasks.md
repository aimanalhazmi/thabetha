...
- [ ] T013 In `backend/app/repositories/base.py`, declare new abstract methods (signatures must match contract bodies in `contracts/api-group-settlements.md`): `create_settlement_proposal(user_id: str, group_id: str) -> SettlementProposalOut`, `get_settlement_proposal(user_id: str, group_id: str, proposal_id: str) -> SettlementProposalOut`, `list_settlement_proposals(user_id: str, group_id: str, status_filter: str | None = None) -> list[SettlementProposalOut]`, `confirm_settlement_proposal(user_id: str, group_id: str, proposal_id: str) -> SettlementProposalOut`, `reject_settlement_proposal(user_id: str, group_id: str, proposal_id: str) -> SettlementProposalOut`, and `sweep_settlement_proposals(group_id: str) -> None`. Also extend the existing `leave_group` ABC docstring to document the new `LeaveBlockedByOpenProposal` 409 path (no signature change).
- [ ] T014 In `backend/app/repositories/memory.py`, implement every method declared in T013. Snapshot logic: select all debts where `group_id == group_id` and `status in {active, overdue}`. Wire `services.netting.compute_transfers` to derive transfers. Materialise the confirmation roster (one row per distinct payer/receiver). Enforce one-active-proposal via dict lookup `{group_id: open_proposal_id}` guarded by the existing `threading.Lock` from Phase 8 — raise `OpenProposalExists` (409) on conflict. `MixedCurrency`/`NothingToSettle` raise 409 with their codes. Lazy sweep on every read updates expired proposals and fires near-expiry reminders (idempotent on `reminder_sent_at`). Atomic settle runs entirely inside a single critical section: per debt, perform two state transitions (`active|overdue → payment_pending_confirmation` then `→ paid`), append two `debt_events`, append one `commitment_score_events` row with `delta=0` and `event_type='settlement_neutral'`. On any exception during settle, restore prior state from a snapshot taken at the top of the critical section, set proposal `status='settlement_failed'` with `failure_reason=type(exc).__name__`, dispatch `settlement_failed` notifications, and swallow the exception so the HTTP response is 200 with the failed proposal. Finally extend `leave_group` to raise `409 LeaveBlockedByOpenProposal` when the leaving user appears in any `status='open'` proposal's transfers for that group.
- [ ] T015 In `backend/app/repositories/postgres.py`, implement every method declared in T013 with parity to memory. Snapshot select: `SELECT id, debtor_id, creditor_id, amount, currency FROM debts WHERE group_id=$1 AND status IN ('active','overdue') ORDER BY id`. Insert proposal with `INSERT … RETURNING id` and rely on the partial-unique index from T004 to surface `OpenProposalExists` (translate `unique_violation` on `one_open_proposal_per_group` → 409). Insert confirmation roster in the same transaction. Lazy sweep is a single function with two `UPDATE … WHERE status='open' AND …` statements. Atomic settle uses one explicit transaction (`BEGIN`/`COMMIT`); on `psycopg.Error` rollback, then in a separate transaction set `status='settlement_failed'` and `failure_reason=str(exc)[:200]`. Use `SELECT … FOR UPDATE` on the proposal row at the top of `confirm_settlement_proposal` to serialise the "last-confirm-triggers-settle" path. Extend `leave_group` parity for `LeaveBlockedByOpenProposal`.

### Backend foundations — API + notifications

- [ ] T016 In `backend/app/api/groups.py`, add the five new routes per `contracts/api-group-settlements.md` §1–§5: `POST /{group_id}/settlement-proposals`, `GET /{group_id}/settlement-proposals`, `GET /{group_id}/settlement-proposals/{pid}`, `POST /{group_id}/settlement-proposals/{pid}/confirm`, `POST /{group_id}/settlement-proposals/{pid}/reject`. Each handler: `repo.ensure_profile(user)`, then `repo.sweep_settlement_proposals(group_id)` for read paths, then call the corresponding repo method, returning the typed Pydantic response. Map `OpenProposalExists`, `MixedCurrency`, `NothingToSettle`, `NotARequiredParty`, `AlreadyResponded`, `ProposalNotOpen`, `StaleSnapshot` to the right HTTP status with `code` in the JSON body. Non-member callers get 404 (not 403) per the contract.
- [ ] T017 [P] In `backend/app/services/whatsapp/templates.py` (or wherever notification copy lives — locate via `grep "debt_cancelled" backend/app/services`), add bilingual copy entries for the seven new notification types (`settlement_proposed`, `settlement_reminder`, `settlement_confirmed`, `settlement_rejected`, `settlement_settled`, `settlement_failed`, `settlement_expired`). Each entry has AR + EN bodies referencing the group name and (where appropriate) the proposed transfer amount. Templates only; in-app notification rows are inserted by the repo methods themselves.

### Frontend foundations

- [ ] T018 [P] In `frontend/src/lib/i18n.ts`, add ~20 new keys for both `ar` and `en` covering: panel CTA (`groups.settle`, `groups.settleNothing`), proposal status labels (`settlement.status.open`, `settlement.status.rejected`, `settlement.status.expired`, `settlement.status.settled`, `settlement.status.failed`), review modal (`settlement.role.payer`, `settlement.role.receiver`, `settlement.role.observer`, `settlement.confirm`, `settlement.reject`, `settlement.expiresIn`, `settlement.transfersHeading`), error codes (`errors.OpenProposalExists`, `errors.MixedCurrency`, `errors.NothingToSettle`, `errors.NotARequiredParty`, `errors.AlreadyResponded`, `errors.ProposalNotOpen`, `errors.StaleSnapshot`, `errors.LeaveBlockedByOpenProposal`), and the seven notification body keys mirroring T017. Bilingual lint guard must pass.
- [ ] T019 [P] In `frontend/src/lib/api.ts`, add typed wrappers under a new `settlements` namespace: `settlements.create(groupId)`, `settlements.list(groupId, status?)`, `settlements.get(groupId, pid)`, `settlements.confirm(groupId, pid)`, `settlements.reject(groupId, pid)`. Errors surface `code` for i18n lookup. Reuse existing fetch + auth header plumbing.
